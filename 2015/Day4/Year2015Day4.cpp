...
void Year2015Day4::Run(std::string filenamePath) {
    RunPart1(filenamePath);
    RunPart2(filenamePath);
}

void Year2015Day4::RunPart1(std::string filenamePath) {
	...
	long i = 0;
	while (true) {
		std::string toBeHashed = base + std::to_string(i);
		std::string hashed = to_hex_string(computeMD5(to_byte_vector(toBeHashed)));
		bool valid = true;
		for (int j = 0; j < 5; j++) {
			if (hashed[j] != '0') {
				valid = false;
...
void Year2015Day4::RunPart2(std::string filenamePath) {
